...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (themeSwitch && themeIconLabel) {
        ...
        // Define o tema padrão como 'light' ao carregar (SEM localStorage)
        aplicarTema('light');
    }
    ...
    const handleFormValidation = (event, form) => {
        ...
        if (form.checkValidity()) {
            ...
            // Simulação de sucesso (sem localStorage)
            if (form.id === 'login-form') {
                alert('Login (simulado) realizado com sucesso!');
                window.location.href = 'index.html'; 
            }
            if (form.id === 'register-form') {
                 alert('Cadastro (simulado) com sucesso! Por favor, faça o login.');
                 window.location.reload();
            }
            if (form.id === 'forgot-password-form') {
                alert('(Simulado) Link de redefinição enviado para o seu e-mail!');
                window.location.href = 'conectar.html';
            }
            if (form.id === 'profile-form') {
                alert('(Simulado) Perfil atualizado com sucesso!');
                window.location.reload();
            }
        }
    ...
    if (filtroTodos) { // Verifica se estamos na página 'minha-lista.html'
    ...
    // CORREÇÃO: Função de aplicar fonte sem localStorage
    ...
    const temSecaoDeCards = document.getElementById('em-alta') || secaoAssistidos; // Reutiliza a variável 'secaoAssistidos'
    ...
    // --- CARREGAMENTO DINÂMICO DE FILMES (CORRIGIDO) ---
    ...
    if (detalhePrincipal) {
        ...
        const preencherPaginaDoFilme = (filme) => {
            ...
            // CORREÇÃO: Preenche os 4 campos de detalhes
